...
> ### ⚠️ STATUS 2026-08-20 — HISTORICAL. Every experiment here is on the RETIRED task.
>
> This document remains the negative-results / ablation record and is still worth citing **as
> history**, but read it knowing:
>
> - **The task it optimises no longer exists.** Component A was reopened as N-1 contingency
>   screening on 2026-08-16. The 4-class target was abandoned because it is a **closed-form
>   function of the observation** — four `if`/`else` rules reproduce it with 100% agreement and
>   **zero disagreements across 315,000 records**. Every point of macro F1 fought for below was
>   fought for on a benchmark that could not distinguish a model from a threshold.
> - **The artifacts are gone.** `gnn_checkpoint_best.pt`, `gnn_checkpoint_leverA.pt`,
>   `gnn_logit_margin.json`, and both classify datasets were deleted on 2026-08-20 (4.81 GB).
>   **The 0.8277 figure is no longer reproducible** — and even before deletion, which of two
>   separate checkpoints produced it was undeterminable.
> - **A normalization bug affected every run recorded here.** All training ran on raw
>   unnormalized features; see `gnn_n1_tightening.md` §3. Some share of the cross-topology
>   degradation reported below is attributable to that rather than to topology transfer.
>
> Current results: [`thesis_findings.md`](../thesis_findings.md) ·
> [`component_d_plan.md`](../component_d_plan.md).
