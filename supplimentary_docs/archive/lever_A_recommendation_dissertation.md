...
> ### ⚠️ STATUS 2026-08-20 — HISTORICAL. Lever A is retired along with the task it calibrated.
>
> This is still a good self-contained decision record and the reasoning about
> operating-point calibration versus representation learning generalises. But:
>
> - **Lever A no longer exists in the code.** It was a per-class logit offset for the 4-class
>   classifier head; that head was removed with the task on 2026-08-16, and
>   `gnn_logit_margin.json` was deleted on 2026-08-20 (recoverable from git history).
> - **The task it served was mis-specified** — the 4-class label was closed-form over the
>   observation (100% agreement, 0 disagreements in 315,000 records), so the confusion it
>   corrected was an artifact of a benchmark that four `if`/`else` statements solve exactly.
> - **The N-1 model has no logit margin and needs none.**
>
> The transferable finding survives and is worth keeping in the write-up: *a calibration tweak,
> not an architecture change, was the only thing that moved the metric* — and it did not
> transfer cross-topology.
