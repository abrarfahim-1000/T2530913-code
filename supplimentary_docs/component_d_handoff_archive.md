...
3. **Dataset size.** No subsampling means far more frames per chronic. Cap with
   `--n_records` / `--max-chronics`; the personal PC has 16 GB RAM and
...
| Machine | Now runs |
|---|---|
| **Personal PC** (Arc B580 12 GB, Ryzen 5 7500F, 16 GB) | forecast dataset generation, GNN training, polarity guard, shield, all tests |
| **Research PC** (RTX 4080 Super 16 GB, 64 GB) | LLM stages only — `translate.py`, `validate.py` |
