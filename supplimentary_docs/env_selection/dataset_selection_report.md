...
> ### ✅ STATUS 2026-08-20 — the decision this report reached is the decision that was built.
>
> Written 20 April 2026, before any component existed, to answer *"which Grid2Op environment?"*
> without training on both. It concluded that the choice was **phase-dependent** —
> `l2rpn_neurips_2020_track1_small` for integration, `l2rpn_wcci_2022` for scale validation.
> That is exactly what happened: the GNN trains on the 36-bus NeurIPS grid and is evaluated
> frozen on the 118-bus WCCI grid. Keep this as the methodology answer to *"why this grid?"*
>
> **Three things it does not know about, which must not be read out of it:**
>
> - **A third environment was added.** `l2rpn_case14_sandbox` (14 buses) became the *unseen
>   smaller* topology, and it is where the model fails hardest — it loses to a single-threshold
>   baseline outright. Scaling **down** cost far more than scaling **up**, which is the opposite
>   of what a report comparing a small grid to a large one would predict.
> - **Its symbolic-schema criterion is void.** §1 and §3.4 score the environments against a
>   knowledge graph that models buses, lines, transformers and generators as first-class nodes.
>   That design was built, measured to be useless, and deleted; the graph is now
>   provenance-centric and topology-agnostic, so "storage units are not modelled" is no longer a
>   difference between environments at all. See [`../study.md`](../study.md) §4.
> - **The task changed.** The probe's coarse event labels (normal / overload / trip) belong to
>   the 4-class target that was retired on 2026-08-16 for being closed-form over the observation.
>   The environments were selected well; the thing they were selected *for* is not what was built.
>
> The `.docx` companion named in the README was never in this folder — the Markdown report is the
> record. Current state: [`../thesis_findings.md`](../thesis_findings.md).
